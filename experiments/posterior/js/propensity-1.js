...
function make_slides(f) {
    ...
    name : "instructions",
     start: function() {
      ...
      var instructions = {
        ...
        communication: "On each trial, you will read a brief excerpt from a conversation between two friends and be asked a question."
      }
  ...
  slides.frequency_judgment = slide({
    ...
    present_handle : function(stim) {
      ...
      var posessive_gender = {
        male: "his",
        female: "her"
      }

      this.observationSentence =  stim.possessive ? 
          stim.character.name + " " + stim.past.replace("their",posessive_gender[stim.character.gender]) + 
              " " + interval_dictionary[stim.interval] + "." : 
          stim.character.name + " " + stim.past + " " + interval_dictionary[stim.interval] + "."
      ...
      var question = stim.possessive ? 
        "How often does "+this.charName + " " +
              stim.verb.replace("their", posessive_gender[stim.character.gender]) + "?" :
        "How often does "+this.charName + " " + stim.verb + "?"
    ...
    button : function() {
      var responses = {
        ntimes: $("#time_frequency").val(),
        time_window: $("#time_comparison").val()
      };
      $(".err").hide();
      // $(".err0").hide();

      if (responses.time_window==null || !(parseFloat(responses.ntimes) > -1))  {
        $(".err").show();
      // } else if (parseFloat(responses.ntimes)%1 != 0) {
      //   $(".err0").show();
      } else {
        this.rt = Date.now() - this.startTime;
        this.log_responses();
    ...
    log_responses : function() {
      ...
      var day_rate = timeTransform[responses.time_window]*parseFloat(responses.ntimes);
      exp.data_trials.push({
        "trial_type" : "propensity_judgment",
        "condition": exp.condition,
        ...
        "ntimes" : parseFloat(responses.ntimes),
        "interval": responses.time_window,
        "rt":this.rt,
        "dayrate":day_rate,
        "log_dayrate": Math.log(day_rate)
      });
    }
  });
