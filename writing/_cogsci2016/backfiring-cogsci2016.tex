...
In 1974, the beleaguered U.S. President Richard Nixon declared to the press corps, ``I am not a crook." (CITE).  On a literal level, this claim argues for Nixon's innocence.  Pragmatically, however, this claim presupposes that someone has accused Nixon of a crime; otherwise, the claim would not be informative (Grice, 1975).  Thus, contrary to Nixon's intentions, a listener unaware of the Watergate scandal could actually \emph{learn} from this utterance that Nixon was associated with corruption.

To date, this phenomenon---that utterances can ``backfire" (i.e., decrease a listener's belief in the content being asserted)---is not well understood.  Instead, the standard Bayesian treatment of learning from testimony has stressed fact that we update our beliefs about the world to be  \emph{in accordance} with information expressed through language \red{(cite)}.  A few studies, however, provide evidence that when utterances are redundant with a listener's prior beliefs, they can imply that the opposite of what is expressed is typically true. Wegner, Wenzlaff, Kerker, and Beattie (1981), for example, demonstrated that newspaper headlines such as ``Bob Talbert not linked with mafia" cause adults to have a more negative impression of Mr. Talbert than they would otherwise, despite explicitly denying incriminating information. Similarly, Gruenfeld and Wyer (1992) showed that uninformative newspaper headlines can actually cause people to believe the exact opposite of what those headlines claim.   After reading the headline ``Robert Kennedy was not planning the assassination of Fidel Castro", for example, adults in their study more strongly believed that Kennedy \emph{was} planning to assassinate Castro. Finally, Kravtchenko and Demberg (2015) recently demonstrated that if a speaker says, ``John paid the cashier!" as part of a story about John's trip to a grocery store, adults infer that John does not typically pay cashiers.

Why do claims like these backfire, while others don't? We suggest that this effect depends on the common ground shared between interlocutors, and the conversational principle that utterances should be both relevant and informative (Grice, 1975), as described in the section below.  In short, when an utterance does not appear on the surface to be informative, interlocutors are able to adjust information that is in common ground to \emph{make} the utterance informative.

In this paper, we formalize pragmatic language models wherein speaker and listener either do or do not share common ground about a particular habit (e.g., a friend's tendency to turn in her homework on time).
These models make three predictions: first, backfiring should occur when common ground is \emph{not} shared between interlocutors, because a speaker may presuppose information that a listener was not aware of, thereby causing the listener to infer that information.  Second, when common ground \emph{is} shared, standard learning from testimony should occur, because the speaker's utterances should be semantically informative for the listener.  Finally, the strength of backfiring should vary according to the strength of the prior beliefs that are redundant with the utterance, since greater redundancy would cause utterances to be less informative.  In Experiment 1, we conceptually replicate \citeA{Kravtchenko2015}, who demonstrated the basic \emph{backfiring} effect for affirmations (e.g., ``John paid the cashier"), and test the first two predictions.  In Experiment 2, we test the third (i.e., the quantitative predictions of our models).
...
Observing an event occur multiple times can be evidence for an underlying cause that could reliably give rise to the event.
...
Seeing a student doing his homework, for example, arguably provides some evidence that the student usually turns in his homework. 
...
A helpful interlocutor can also communicate knowledge about an event.
When a speaker accurately represents the listener's prior knowledge (e.g., the listener and speaker are close friends; or the listener makes explicit her prior knowledge to the speaker by asking, ``Did your student turn in his homework today?''), communicating the event (e.g. ``My student turned in his homework today'') can lead the listener to believe the underlying habit (here, that the student \emph{typically} turns in his homework) more strongly. 
However, when a speaker misrepresents the listener's knowledge (e.g., the listener has forgotten something the speaker believes her to know, or the listener is overhearing a conversation between friends), communicating the event might lead the listener to believe the habit is \emph{less} likely.  If the student did usually turn in his homework, then the utterance would be uninformative.  
...
These inferences can be readily incorporated into Rational Speech-Acts framework \cite{Frank2012, Goodman2013}:, as a lifted variable inference akin to the treatment by \citeA{Degen2015} and discussed in detail in \citeA{GoodmanLassiter}.
%
\begin{eqnarray}
...
The model predicts that when a speaker does not represent a listener's beliefs accurately, utterances affirming an event will be interpreted as expressing atypical behavior.  The statement, ``My student turned in his homework today,'' for instance, should suggest that the student does \emph{not} usually turn in his homework (Figure \ref{fig:schematic-model}).
...
If a listener explicitly asks, for example, ``Did your student turn in his homework today?'' then the speaker will learn that the listener is unsure of the student's habits.  As a result, the response, ``My student turned in his homework today,'' will become semantically informative, and will no longer presuppose that the listener knows about the student's habits.
...
Another area in which we wouldn't expect linguistic evidence to backfire is observational learning.
...
In our first experiment, we tested the qualitative predictions of our pragmatic model that:

	1. backfiring should occur in conversational contexts in which a speaker presupposes common ground that a listener actually lacks (e.g., when a speaker states, ``My student turned in his homework today," to a listener who is not aware of the student's typical habits)

	2. backfiring should \emph{not} occur in conversational contexts in which a listener makes it clear that she is uncertain about a habit (e.g., when the listener explicitly asks the speaker if the student turned in his homework)

	3. backfiring should \emph{not} occur in non-conversational contexts (e.g., when the listener simply observes that the speaker's student turned in his homework)
...
In the \textbf{prior} condition, no additional information was provided, and participants were asked: ``Who/what is the person/business related to Person B?'' (e.g., Who is Molly's officemate tutoring?).  Participants selected one of the two options (e.g., Tom or Jim) and then rated their confidence in their choice with a slider bar.

In the three experimental conditions (backfiring, ignorant speaker, and observation), participants read additional information that varied according to the condition and then answered the same question.

In the \textbf{backfiring} condition, a brief communicative utterance was added after the description of the two options.  In the officemate scenario, for example, participants read, ``In their office, Molly's officemate says to her, `My student turned in his homework on time today.'"

In the \textbf{ignorant speaker} condition, Person A explicitly stated that they did not know whether an event happened.  In the officemate scenario, this was: ``In their office, Molly helps her officemate fill out a daily report card for her student, while her officemate files away some papers.  Molly reads out loud from the report card,`�Did your student turn in his homework on time today?'  Her officemate replies, `Yes. My student turned in his homework on time today.'"

In the \textbf{observation} condition, an observation was added after the two options.  Participants read, for example, ``In their office, Molly notices some papers on her officemate's desk.  Her officemate's student turned in his homework on time today."
...
We fit the \emph{endorsement score} data using a generalized mixed-effects model with \red{fixed effects of X and random effects of X?} random by-participant effects of intercept and random by-item effects of intercept\footnote{This was the maximal-model that our data permitted.}. 
As predicted, participants endorsed the likely response \emph{less} in the Backfiring condition (M=-0.09; bootstrapped 95\% CI [-0.15, -0.03]) than the in the Prior condition (M=0.18 [0.14,0.22]), $\beta = -0.24; SE = 0.03; t(1794) = -8.3; p<0.001$.
...
No such trend was observed for the Ignorant Speaker condition (M=0.18 [0.13, 0.24]). 
...
Critically, participants were significantly \emph{less} likely to choose the ``likely''� option in the Backfiring condition (M=0.46 [0.41, 0.50]) than in the Prior condition (M=0.71 [0.67,0.75]) , $\beta = -1.42; SE = 0.15; p < .001$.  
Interestingly, however, participants were \emph{no more} likely to choose the ``likely'' option in either the Observation (M=0.71 [0.68, 0.75]) or Ignorant Speaker (M=0.68 [0.63,0.71]) conditions than in Prior (Figure \ref{fig:expt1-all}; middle panel).

Finally, since participants are not given any helpful information in the Prior condition, we expected the confidence rating data to be lower in the Prior condition than in the three experimental conditions.
In all conditions, this was the case: Prior 0.39 [0.36, 0.42] vs. Observation 0.60 [0.58, 0.62], Ignorant Speaker 0.60 [0.58, 0.61], Backfiring 0.62 [0.61, 0.64] (Figure \ref{fig:expt1-all}; right panel). \red{do we need this paragraph?  could we just do endorsement + FC?}

Overall, as predicted, participants in the Backfiring condition interpreted utterances as expressing less typical events.  If an officemate states, ``My student turned in his homework on time today," then the student must not typically turn in his homework on time.  However, we also expected participants in the Observation and Ignorant Speaker conditions to interpret the target statements as evidence that the relevant behavior \emph{does} typically occur, which we did not find.  Upon reading an observation that a student turned in his homework on time, for instance, we expected participants to be more likely to believe that the student usually turns in his homework on time, relative to baseline.  Instead, participants in the Observation and Ignorant Speaker conditions did not differ from those in the Baseline condition.  There are two possible explanations for this lack of differences: 1) prior beliefs could have already been at ceiling, making it difficult for linguistic and observational evidence to influence responses, and 2) participants might have needed more than one piece of evidence to make a strong generalization about behavior. \red{this is not well written}
...
In this experiment, we test the prediction of our model that the strength and presence of backfiring should systematically vary according to the strength of prior beliefs.  Specifically, utterances that are strongly redundant with prior beliefs (e.g., ``John wore shoes today") should backfire more than utterances that are less redundant (e.g., ``John went for a hike today"), by virtue of being less semantically informative.

\subsection{Method}

\subsubsection{Participants}
Participants were XXX

\subsubsection{Materials and procedure}
Participants participated in one of three conditions: the \emph{Backfiring} condition, the \emph{Observation} condition, or the \emph{Baseline} condition.

In the \emph{Backfiring} condition, participants were told that they would read a series of 27 brief excerpts from conversations.  Each excerpt consisted of a single utterance: one person describing an event to a friend (e.g., ``Kelly wore socks today").  After reading each utterance, participants indicated how frequently they believed the agent in the utterance engages in the activity (e.g.,``How often does Kelly wear socks?").  Participants responded by stating \emph{X} times per week, month, year, or five years, where \emph{X} could be any number.

The \emph{Observation} condition was identical to the \emph{Backfiring} condition, except participants were told that they would read a series of random facts about people rather than excerpts from conversations.

In the \emph{Baseline} condition, participants were simply asked to rate how often a typical person does each of the actions used in the \emph{Backfiring} and {Observation} conditions, without having read anything else beforehand.  We asked participants to assume that the typical person had done the action at least once, to avoid... \red{we should justify why we used the exact baseline we did}

\subsection{Results}
We coded responses as...

\section{General Discussion}

Our experiments serve as conceptual replications of previous work (CITE) demonstrating that adults try to avoid redundancy in conversation by adjusting information that is in common ground between interlocutors.  Ultimately, this can result in an utterance ``backfiring", or implying that the content expressed is typically not true.  If a person notes, ``My roommate washed her dishes today," for example, then a listener who does not know the roommate will likely infer that the roommate does \emph{not} typically was her dishes; otherwise, the utterance would not be informative (CITE).

We have also made several novel contributions to our understanding of this effect.  First, we have shown that this effect disappears when an interlocutor elicits the target utterance by expressing uncertainty.  If a person asks their friend, ``Did your roommate wash her dishes today?" and the friend responds, ``Yes, my roommate washed her dishes today," then the utterance no longer presupposes any shared knowledge about the roommate.  Instead, the utterance becomes semantically informative for the listener, and there is no need to infer that the roommate does not usually wash her dishes.  Thus, backfiring critically depends on a misunderstanding of information that is in common ground.  The effect also disappears when the proposition is framed as an observation rather than a communicative utterance (e.g., ``She saw that the roommate washed her dishes today").  When a statement is removed from a conversational context, there is no longer any pressure to be maximally informative.  As a result, even if the statement is redundant with adults' prior beliefs, they will not adjust information in common ground to make the statement informative.  Second, we have demonstrated that the strength of backfiring depends on the strength of prior beliefs.  Hearing, ``Jane wore shoes today," for example, will result in stronger backfiring for a person who does not know Jane than will, ``Jane wore sandals today," because wearing shoes more strongly \emph{goes without saying}.  Finally, we have presented a formal pragmatics model that predicts each of these behavioral results.
...
In addition to contributing to our knowledge of language processing, this work has practical relevance.  
