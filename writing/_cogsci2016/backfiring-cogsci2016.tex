...
Why should some utterances serve as evidence for a given habit, while others serve as evidence against it?  
We present a formal language understanding model that is able to revise information that is common ground when an utterance is uninformative with respect to prior beliefs.  
...
However, the fact that Nixon said this, which---for a president, at least---should go without saying, we take his utterance to imply that he is actually somehow associated with corruption.
...
Observing an individual perform an action can be taken as evidence that that individual \emph{typically} performs that action, or has the habit $h$ of doing behavior $b$. 
...
When a speaker accurately represents the listener's prior knowledge (e.g., the listener and speaker are close friends; or the listener makes explicit her prior knowledge to the speaker by asking, ``Did $b$ happen?'', ``Did your student turn in his homework today?''), communicating the behavior (here, ``My student turned in his homework today'') can lead the listener to believe the underlying habit (here, that the student \emph{typically} turns in his homework) more strongly. 
...
Here, we would expect observing the student turning in his homework to be taken as evidence that he usually \emph{does} do his homework. 
...
We use previously collected data from an unrelated experiment to estimate these model components. 
We then model the data from each of the backfiring, observation, and baseline conditions of Expt.~2 using the backfiring RSA model (Eq.~\ref{eq:L1}) and the observational Bayesian model (Eq~\ref{eq:bayes}) that both uses and neglects the observed data, respectively. 
...
We assume that each item had two components corresponding to individuals who do the action \emph{relatively often} (with rate $\lambda_h$) and those who do the action \emph{relatively infrequently} (rate $\lambda_{\neg h}$).
We further infer the mixture parameter $\theta$ to determine how common it is to do the action relatively often.
