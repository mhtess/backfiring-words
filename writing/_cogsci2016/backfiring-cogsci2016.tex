...
For example, for a public figure to assert \emph{Nixon was not a crook}, the speaker must believe that the listener already knows that \emph{somebody} had piqued the professor \cite{Clark1977}.  But if the listener actually lacks this knowledge, the exchange does not fall apart; instead, the listener can accommodate the utterance by adjusting the common ground between herself and the speaker, inferring that it must be commonly known that someone had piqued the professor (von Fintel?).  Thus, upon hearing Nixon state, ``I am not a crook," a listener unaware of any scandals surrounding Nixon may adjust the information in common ground to include an accusation that Nixon \emph{was} involved in criminal activity.  

One important prediction of this theory, then, is that this backfiring effect should disappear if the listener's uncertainty about what is in common ground is made explicit.  If, for instance, an official needed to know whether Nixon was a crook as part of a background check for serving as President, and Nixon replied, ``No, I am not a crook," then the utterance is established as semantically informative for the listener.  As a result, it no longer presupposes any accusation against Nixon.  Another important prediction is that the strength of the backfiring effect should vary according to the strength of the prior beliefs that are redundant with the utterance.  The more strongly one believes that people by default are not crooks, for example, the less informative the statement, ``I am not a crook," should be.

In the present study, we directly test these predictions.  In Experiment 1, we conceptually replicate previous findings that adults try to avoid semantic redundancy in conversation by adjusting information that is in common ground between speakers.  We also show that \red{something about when common ground is aligned}.  In Experiment 2, we show that \red{the strength of the backfiring effect varies according to the strength of the redundant prior beliefs.  Do we need something about observation?}.

\red{state that we don't actually rely on utterances being unambiguously uninformative?  instead, we show that any utterance can backfire, so long as there is a misalignment regarding common ground?  maybe add this to discussion?}
...
In this experiment, we tested the qualitative predictions of the pragmatic model that:

- backfiring should occur in conversational contexts in which a speaker presupposes common ground that a listener actually lacks (e.g., the speaker states, ``My student turned in his homework on time today," when the listener is not aware of the student's typical habits)

- backfiring should \emph{not} occur in conversational contexts in which a listener elicits the speaker's utterance by making it clear that she is uncertain about an event (e.g., when the listener explicitly asks the speaker if the student turned in his homework on time)

- backfiring should \emph{not} occur in non-communicative contexts (e.g., when the listener simply observes that the speaker's student turned in his homework on time today)
...
Interestingly, however, participants were \emph{no more} likely to choose the ``likely'' option in either the Observation (M=0.71 [0.68, 0.75]) or Ignorant Speaker (M=0.68 [0.63,0.71]) conditions than in Prior (Figure \ref{fig:expt1-all}; middle panel).  There are two possible explanations for this lack of differences: 1) prior beliefs were already at ceiling, making it difficult for linguistic and observational evidence to influence responses and 2) participants might have needed more than one piece of evidence to make a strong generalization about behavior.
...
\subsection{Quantitative model evaluation and discussion?}
