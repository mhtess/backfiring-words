...
%%\citeA{Wegner1981}, for example, demonstrated that newspaper lines such as "â‚¬Å%“Bob Talbert is not linked to the Mafia" [CHECK] cause adults to have a more negative impression of Bob than they would otherwise, despite not explicitly asserting any incriminating information--indeed, despite denying incriminating information.  Here, since the default assumption is that people are not typically linked to the mafia, this line is not immediately informative.  As a result, adults construct a context that \emph{would} make the line informative, namely, one in which Bob is known to be a bad person.  
%%\citeA{Gruenfeld1992} went even further to show that uninformative newspaper lines can actually cause people to believe the exact opposite of what those lines claim.  After reading the line "â‚¬Å%“Robert Kennedy was not planning the assassination of Fidel Castro"â‚¬Â, for example, which denies a proposition that adults \emph{a priori} should not believe, adults in their study more strongly believed that Robert Kennedy \emph{was} planning to assassinate Fidel Castro.
...
%\citeA{Wegner1981}, for example, demonstrated that newspaper lines such as ``Bob Talbert not linked with mafia'' cause adults to have a more negative impression of Mr. Talbert than they would otherwise, despite denying incriminating information.  Similarly, \citeA{Gruenfeld1992} showed that uninformative newspaper lines can actually cause people to believe the exact opposite of what those lines claim.  
%After reading the line ``Robert Kennedy was not planning the assassination of Fidel Castro'', for example, adults in their study more strongly believed that Kennedy \emph{was} planning to assassinate Castro.  
...
We first compute the mean rate from the baseline condition and use that as a predictor (after centering) crossed with condition (Backfire vs. Observational) in a mixed-effects linear regression, with random by-participant effects of intercept and base rate and by-item effects of intercept and condition\footnote{This is the maximal mixed-effects structure.}. 
Frequencies implied an observation were significantly larger than the frequencies implied in the backfiring condition ($\beta = 0.35, SE = 0.13, t(67.6) = 2.56, p = 0.01$), and there was a trend for that difference to be larger as the base rate increased ($\beta = 0.09, SE = 0.05, t(60.4) = 1.92, p = 0.059$). In addition, there was a significant main effect of base rate on the implied rate ($\beta = 0.57, SE = 0.09, t(27.1) = 6.6; p<0.01$).
