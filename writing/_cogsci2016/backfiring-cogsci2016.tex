...
Observing an individual perform an action can be taken as some evidence that that individual \emph{typically} performs that action, or has the habit $h$ of doing behavior $b$. 
...
\end{eqnarray}
where $\lambda_{h} > \lambda_{\neg h}$. The model is implemented in the probabilistic programming language WebPPL \cite{dippl}, and a simple version can be seen at \mht{add forest page}.
...
\mht{I don't believe our method was. Probably the biggest difference was theirs implicitly relies on prior expectations whereas we setup the contrast explicitly.}  We performed a power analysis using a pilot experiment run on a subset of items to determine that we should collect 120 participants to achieve 0.95 power. 
...
We have presented a formal model for how an utterance can \emph{backfire}, or imply that its content is typically not true.
...
Our work is therefore of \emph{practical} in addition to theoretical relevance.  
...
Stating, ``Girls can do math'', for instance, potentially signals to the listener that the speaker thinks the listener doesn't believe it is true. Why the speaker might think that---the logic goes---is perhaps because many people believe that girls \emph{cannot} do math and there are legitimate reasons to hold this belief (e.g., perhaps girls tend to be less intelligent than boys).  
As a result, these listeners might walk away doubting girls' ability to do math more than they would have otherwise.  One way of avoiding this possibility, as our work suggests, is to establish common ground explicitly with listeners before presenting an argument that could potentially be considered uninformative an audience.
