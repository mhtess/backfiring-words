...
The statement, ``Jane went for a hike today," could likely serve as evidence that Jane likes to go on hikes.  However, this does not hold true for all statements.  Consider the statement, ``Jane wore shoes today."  In this case, the listener will likely make the inference that Jane does \emph{not} typically enjoy wearing shoes.  Why should some utterances serve as evidence for a given behavior, while others serve as evidence against it?  We suggest that when an utterance is redundant with the prior beliefs of the listener (here, that most people wear shoes), adults will revise information that is in common ground (here, that Jane typically wears shoes) so that the utterance becomes informative.  In Experiment 1, we show that this inference does not occur when common ground is mutually understood between interlocutors, or when a statement is removed from a conversational context.  In Experiment 2, we show that the \emph{strength} of this inference varies according to the strength of the listener's prior redundant beliefs.  Finally, we present a computational model that predicts these results.
