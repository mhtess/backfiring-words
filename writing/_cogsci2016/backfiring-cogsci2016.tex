...
We have replicated previous findings that adults interpret utterances so that they are not redundant with a listener's preexisting beliefs (CITE).  Upon hearing, "My student turned in his homework on time today," for instance, adults will learn not only that the student turned in his homework on time, but also that the student must not typically turn in his homework on time.  This inference is based on the conversational agreement that utterances should be maximally informative \cite{Grice1975}.  If the listener were already aware that the student always turns in his homework on time, in other words, then this utterance would be pragmatically infelicitous.

We also expanded upon prior work in two important ways: 1) we demonstrated that adults do not make this inference when the utterance is produced in a non-communicative context (e.g., two friends filling out a survey together), and 2) we provide a computational model that predicts this inferential process.  [Brief description of the model.]

In addition to being important for developing theories of language processing, this work has practical relevance.  [[Recent efforts to persuade the general public that there is no causal relation between vaccines and autism, for example, include articles titled, "Vaccines do not cause autism".]]

It is therefore important to determine the kinds of inferences people might make from these utterances.  Do they infer that most people must believe otherwise?  Do they generally doubt the content of the proposition?
=======
...
This opens the door for manifestly different types of conceptual learning from language. <---This is a good point, but I feel like we'll end up focusing *not* on language (and instead on the relation between irregular events and traits)... which maybe is ok, I don't know.
