...
Overall, participants in the \emph{Backfiring} condition interpreted utterances as expressing atypical events relative to the other 3 conditions as predicted by our models and conceptually replicating \citeA{Kravtchenko2015}.  If an officemate states, ``My student turned in his homework today," then the student must not typically turn in his homework.  However, we also expected participants in the \emph{Observation} and \emph{Instructive} conditions to interpret the target statements as evidence that the relevant behavior \emph{does} typically occur more often than in the \emph{Baseline} condition, which we did not find. 
...
In our second experiment, we aimed to elicit reactions to utterances about human behaviors covering a wide range of likely frequencies of action, as well as to simplify and generalize the experimental paradigm. 
...
Our model predicts a continuum of the degree of backfiring ranging from utterances about events that are \emph{a priori} very likely (e.g., ``John wore shoes today") to events that are very unlikely (e.g., ``John climbed a mountain today").
...
=======
>>>>>>> cf96f6243fcaa0bcf2c3aa7ab0fd8e1a4951aa4c
...
In the \textbf{Observation}, participants were told on each trial that they would be given a random fact about a person doing an action and be asked to judge how often that person does the action. On each trial, participants read ``\textsc{person} did X today'', where X was an action (e.g., ``Stephen went for a run today.''). Participants were asked, ``How often does \textsc{person} do X?''  (e.g., ``How often does Stephen go for runs?'') Participants responded ``M times per K,'' by entering a number for M and choosing K from a drop-down with options week, month, year, and 5 years. 

In the \textbf{Backfiring} condition, participants were instead told that they would read excerpts from conversations between friends.  
On each trial, participants read ``You overhear two friends talking. One of them says to the other, `\textsc{person} did X today'.''

In the \textbf{Baseline} condition, participants were asked to rate how often a typical person performs the action, without any other information.
...
We present a formal model for understanding how language can backfire,  or implying that the content expressed is generally not true. 
The model derives predictions by revising common ground when a speaker utters something which would otherwise be semantically uninformative.
In Expt.~1, we validate that this prediction depends upon the information being communicated and not observed as well as a misalignment of common ground between speaker and listener in conversation. This experiment also serves as a conceptual replication of \citeA{Kravtchenko2015} and adds to the body of literature demonstrating that adults try to avoid redundancy in conversation by adjusting information that is in common ground between interlocutors.  
