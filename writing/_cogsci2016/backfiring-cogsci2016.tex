...
In the present study, we directly test these predictions... First, we conceptually replicate previous findings that adults try to avoid semantic redundancy in conversation by adjusting information that is in common ground between speakers.  Second, we show that prior... \red{state that we don't actually rely on utterances being unambiguously uninformative?  instead, we show that any utterance can backfire, so long as there is a misalignment regarding common ground}
