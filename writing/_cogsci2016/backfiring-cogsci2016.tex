...
When this common ground is not shared, a listener can use a speaker's utterance in conjunction with conversational principles to back out what was assumed by the speaker.
...
Here, we formalize common ground inference using the case of when a speaker's utterance refers to something that should already be in common ground.
The formal pragmatics model makes clear that this kind of utterance will backfire (i.e. imply the opposite) when the interlocutors do not share common ground, but not when that they do. 
...
The human capacity to generalize from examples such as these is the foundation for learning about the world's categories \red{[cite markman]} and people \red{[cite learning about people]}. 
...
If a listener is uncertain about what is in common ground, she can use conversational principles  \cite{Grice1975} and the speaker's utterance to infer what the speaker presupposed.
...
\centering
    ...
    \caption{How does a learner or listener interpret evidence of an event, when the agent has uncertainty about what could have given rise to the event (red or blue)? 
...
In a conversational setting, knowledge of such a person's habits may or may not be in common ground, and the speaker and listener may or may not be aware of this \cite{Clark1977, Clark1996}. 
...
Friends may have a mutually-understood common ground about a third person, but one that an overhearer might not have. 
This situation also will arise when the one of the friends assume a bit of knowledge that the other person doesn't have (e.g. because she forgot).
...
If a listener thinks that the speaker is presupposing some bit of knowledge (e.g. because the listener has forgotten this bit of information or because the listener is overhearing a conversation), she can use the speech-act to help resolve what the speaker believes to be in common ground.
...
\centering
    ...
    The opposite is true when words backfire: The stronger the \emph{a priori} belief in the habit, the utterance conveys more evidence against that habit. }
...
If a listener explicitly asks about the event occurring (e.g. ``Did your roommate wash her dishes today?''), the speaker no longer needs to be presuppose the listener knows about the habit to make the utterance informative. 
...
These predictions are borne out in a standard model of Bayesian learning (Figure \ref{fig:schematic-model}, red bars).
...
We measured interpretations both when the speaker believes the habit is in common ground and when the listener makes explicit that she this is not in common ground.
...
In the \textbf{observation} condition, Person B observes evidence that \emph{X happened.}.



%On one trial, for example, participants read the following scenario: â€œâ€  Participants then had to decide who they thought the officemateâ€™s student was--Tom or Jim--without any other information and rate their confidence in their decision on a sliding scale.  All trials had the same structure: a character having trouble remembering information, and then a description of two options, one of which always engages in a behavior, and one of which only occasionally engages in a behavior.
%
%Pragmatic condition. The Pragmatic condition was identical to the Baseline condition, except a brief utterance was added to the vignette after the description of the two options.  In the officemate scenario, for example, participants read, â€œIn their office, Mollyâ€™s officemate says to her, â€˜My student turned in his homework on time today.â€™â€  We predicted that in this condition adults should be more likely to state, relative to baseline, that the officemateâ€™s student is the one who only occasionally turns in his homework on time, since that context would make the utterance informative.
%
%Literal condition. The Literal condition was the same as the Pragmatic condition, except an observation was added after the description of the two options rather than an utterance.  In the officemate scenario, this observation was, â€œIn their office, Molly notices some papers on her officemate's desk. Her officemate's student turned in his homework on time today.â€  Here, the same content is expressed--the student turned in his homework on time today--but it is no longer part of a conversation.  Thus, adults should be somewhat biased, relative to baseline, to select the student who always turns in his homework on time, since this evidence is most consistent with that behavior.
%
%Speaker Manipulation condition.  The Speaker Manipulation condition was also the same as the Pragmatic condition, except the utterance added was no longer in a communicative context(??).  Participants read, for example, â€œIn their office, Molly helps her officemate fill out a daily report card for her student, while her officemate files away some papers.  Molly reads out loud from the report card, â€˜Did your student turn in his homework on time today?â€™  Her officemate replies, â€˜Yes. My student turned in his homework on time today.â€™â€  Because the sentence, â€œMy student turned in his homework on time today,â€ is now a response to a question on a report card rather than a spontaneous communicative act, it does not have any pragmatic implications.  As a result, participants should again be somewhat biased to select the student who always turns in his homework on time.
...
We first analyze the data separately in terms of (1) the proportion of participants who selected the more \emph{likely} option given an observation (e.g. the roommate who \emph{always} does her dishes) and (2) the confidence ratings. 
We then analyze a composite endorsement score, which incorporates the confidence ratings by scaling the response (either +1 or -1) by confidence. 

We fit the forced-choice data using a generalized mixed-effects model with random by-participant effects of intercept and random by-item effects of intercept \footnote{This was the maximal-model that our data permitted.}. 
Critically, participants were significantly \emph{less} likely to choose the ``likely''� option in the Misaligned Common Ground condition (M=0.46; bootstrapped 95\% CI [0.41, 0.50]) than in the Prior condition (M=0.71 [0.67,0.75]) , $\beta = -1.42; SE = 0.15; p < .001$.  
We expected the observation and speaker condition to be more likely 
Interestingly, participants were no \emph{more} likely to choose the ``likely'' option in either the observation (M=0.71 [0.68, 0.75]) or aligned CG (M=0.68 [0.63,0.71]) conditions as compared to the Prior (Figure \ref{fig:expt1-all}; left panel).
...
As a manipulation check, we expected the confidence rating data to be higher in the three experiment conditions (observation, CB aligned / misaligned) than in the prior condition. 
In all conditions, this was the case: Prior 0.39 [0.36, 0.42] vs. Observation 0.60 [0.58, 0.62], CG Aligned 0.60 [0.58, 0.61], CG Misaligned 0.62 [0.61, 0.64] (Figure \ref{fig:expt1-all}; middle panel).
