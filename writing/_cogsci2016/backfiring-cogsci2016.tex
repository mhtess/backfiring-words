...
If you just met Jane and her friend says, ``Jane went on a hike today," you might think Jane is the kind of person who likes to go on hikes.  
...
The model predicts that this ``backfiring'' inference relies on the fact that prior probability of the event occurring was high and that 
the speaker did not have to say anything---in effect, that the utterance could go without saying. 
The model predicts an utterance will \emph{not} backfire when the speaker is asked a polar question, and when the event is observed rather than relayed in conversation, which we manipulate in Experiment 1.
The model also predicts degrees of backfiring contingent on the \emph{a priori} beliefs about the event under discussion, which we explore in Experiment 2. 
...
Indeed, the standard Bayesian treatment of learning from testimony has stressed that we update our beliefs about the world to be  \emph{in accordance} with information expressed through language, and even more strongly than if we'd observed the same fact on our own \cite{shafto2012learning}.
...
The model predicts backfiring when the assertion is \emph{a priori} very likely. It also predicts two conditions under which backfiring should occur.
First, when the evidence comes from a simple observation, rather than from a helpful speaker, it should provide positive (not negative) evidence.
Second, if the speaker \emph{must} make the utterance, then it should no longer backfire:  making an assertion when that situation is already likely only runs a danger of being uncooperative if the speaker had the \emph{option} of keeping silent (and thus letting the prior rule). 
We can arrange for the speaker to have no option of silence by asking a polar question: ``Are you a crook?'' ``I'm not.''

In Experiment 1, we test the model predictions by arranging for the same (likely) fact to be either spontaneously uttered (yielding backfiring), uttered as the response to a polar question, or as a non-social observation. 
We also vary the prior probability of the fact in question.
In Experiment 2, we further explore the quantitative variation predicted by the model as a function of shared prior beliefs.
...
Observing an individual perform an action can be taken as evidence that they \emph{typically} perform that action---they have the habit $h$ of doing behavior $b$. 
...
This {\bf inference from observation} follows from Bayes' rule:
\begin{eqnarray}
P_{Obs}(h \mid b) \propto P(b \mid h) P(h) \label{eq:bayes}
\end{eqnarray}
The observation 
...
What if, instead of an observation, we hear this fact spoken by a helpful listener? This can be formalized as Bayesian updating again, but based on the observation of a speaker choosing the utterance, $u$:
\begin{equation}
P_{L_1}(b, h \mid u) \propto P_{S_1}(u \mid b, h)\cdot P(b \mid h) \cdot P(h) \label{eq:L1}.
\end{equation}
Here the listener infers both $b$ and $h$, but if she is interested only in the habit: $P_{L_1}(h \mid u) = \sum_b P_{L_1}(b, h \mid u)$.
We imagine a speaker who attempts to convey $b$ to a literal listener, assuming the listener to knows $h$:
\begin{eqnarray}
&&P_{S_1}(u \mid b, h) \propto  P_{L_0}(b \mid u, h)^{\alpha} \label{eq:S1}\\
&&P_{L_0}(b \mid u, h)\propto \denote{u}(b) \cdot P(b \mid h) \label{eq:L0}
\end{eqnarray}
The resulting model is a variation on the  Rational Speech Acts framework \cite{Frank2012, Goodman2013}, in which a particular fact (here the existence of the habit, $h$) 
is presumed to be common ground by the speaker, but reasoned about by the pragmatic listener. This treatment follows \citeA{Degen2015}, within the approach discussed more generally in \citeA{GoodmanLassiter}.

The predictions of the $L_1$ model in Equation \ref{eq:L1} critically depend on the set of alternative utterances, $u$, that the speaker is free to make.
For a spontaneous, unconstrained utterance, we assume the possibility of remarking on the event with an affirmation or a negation --- ``b'' or ``not b''--- as well as the possibility of not remarking on the situation, a \emph{null} utterance that has no information content. 
As we see in Figure \ref{somefig} \ndg{make fig}, where this setup is labeled as the {\bf backfiring inference}, the model makes the critical prediction that the utterance  of ``b'' decreases belief in the habit relative to the prior and relative to an \emph{observation} of $b$.
However, if we imagine that the speaker is forced to make an affirmative and negative statement, without the opportunity to stay silent, as would be the case after being directly asked the question ``b?'', then backfiring disappears.
...
\mht{Maybe explain continuous version first and then note the special case when there is $h$ and $\neg h$}

To flesh out the model, we have to describe the priors $P(h)$ and $P(b|h)$
We can think of behaviors as events that are distributed in time according to a Poisson distribution,  $b \sim \text{Poisson}(\lambda)$, with a rate $\lambda$ that is influenced by whether or not the person has the habit. 
In the binary case, if the habit $h$ is present, the behavior occurs with a high rate  $\lambda_{h}$ and if the habit is absent, the rate is lower $\lambda_{\neg h}$,
where $\lambda_{h} > \lambda_{\neg h}$. 
In the continuous case, the listener has prior beliefs about the likely rates $\lambda \sim P(\lambda)$, which we will explore in Expt.~2.
%\ndg{any reason not to just use the version you'll need for expt 2?}
...
The trio of models described above---observation and pragmatics with and without the option to remain silent---predict that utterances should backfire when an \emph{a priori} likely fact is heard as a spontaneous utterance, but not when it is a simple observation or the response to a polar question. 
Predictions depend on the prior probability of the event occurring (given the habit) being relatively high; thus, we construct two variants of each scenario: one for which the behavior is very likely and one for which the behavior is less likely (e.g., a student who \emph{always} turns in his homework on time vs. one who \emph{only occasionally} does so).
This is similar in spirit to \citeA{Kravtchenko2015}, though we include observation and question conditions, inspired by the model. In addition, we use a two-alternative forced choice to examine the backfiring effect directly. 
...
\begin{figure*}
%\centering
    \includegraphics[width=\textwidth]{expt2-scatter-witems-2}
    \caption{
    Left: Inferred frequency as a function of \emph{a priori} frequency (see text) for the three experimental conditions. When the prior probability of an event is high, ... \mht{\emph{COMMUNICATIVE BACKFIRES}} ... 
    X-axis on log scale.
    Right: \mht{\emph{Y-AXIS SHOULD BE AVERAGE DAILY RATE}, n times / day} By-item plots. When the \emph{a priori} frequency is high, backfiring effects are the largest. 
    }
  \label{fig:expt2}
\end{figure*}
