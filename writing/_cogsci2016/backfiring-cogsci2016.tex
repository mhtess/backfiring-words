...
The model predicts backfiring when the assertion is a priori very likely. It also predicts two conditions under which backfiring should reverse.
...
In Experiment 1 we the model predictions by arranging for the same (likely) fact to be either spontaneously uttered (yielding backfiring), uttered as the response to a polar question, or as a non-social observation. We also vary the prior probability of the fact in question.
...
For now, we assume that if the habit $h$ is present, the behavior occurs with a high rate  $\lambda_{h}$ and if the habit is absent, the rate is lower $\lambda_{\neg h}$,
