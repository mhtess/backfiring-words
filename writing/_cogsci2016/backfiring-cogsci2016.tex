...
Another area in which we wouldn't expect evidence to backfire is observational learning.
...
In this experiment, we test the quantitative predictions of our models.  Specifically, we ask whether the strength and presence of backfiring varies systematically according to the strength of prior beliefs.  Our models predict that utterances that are strongly redundant with prior beliefs (e.g., ``John wore shoes today") should backfire more than utterances that are less redundant (e.g., ``John went for a hike today"), by virtue of being less semantically informative.
...
In the \emph{Baseline} condition, participants were simply asked to rate how often a typical person performs each of the actions used in the \emph{Backfiring} and \emph{Observation} conditions, without having read anything else beforehand.  We asked participants to assume that the typical person had performed the action at least once, so that the priors / to avoid the possibility that... \red{we should justify why we used the exact baseline we did}
