...
If you just met Jane and a friend tells you, ``Jane went for a hike today", you might think Jane is the kind of person who likes to go on hikes.  
...
We present a formal model of language understanding that is able to revise information that is common ground when an utterance is uninformative with respect to prior beliefs.  
...
\section{Experiment 1: \red{Backfiring as a function of communicative context?}}
...
We tested the backfiring effect by comparing the rate implied by the statement in the \emph{Backfiring} condition to the rate implied in the \emph{Observation} condition, as a function of the baseline rate.
We first compute the mean rate from the baseline condition and use that as a predictor crossed with condition (Backfiring vs. Observation) in a mixed-effects linear regression, with random by-participant and by-item effects of intercept.
After centering the predictors, we observe a main effect of baseline rate ($\beta = 1.32; SE = 0.18; t(25) = 7.1; p <0.001$), meaning that as the baseline rate increases, so too do the rates in the \emph{Observation} and \emph{Backfiring} conditions. We also observe a main effect of condition ($\beta = -0.24; SE = 0.05; t(75) = -4.3; p <0.001$), with statements in the \emph{Backfiring} condition in general implying lower rates than those in the  \emph{Observation} condition. Crucially, we observed an interaction between baseline rate and condition ($\beta = -0.85; SE = 0.12; t(1975) = -7.08; p <0.001$), such that the differences between the rates in the \emph{Backfiring} and \emph{Observation} conditions increased \emph{as the baseline rate increased}. \red{is this what you wanted to say?}
...
We used previously collected data from an unrelated experiment to estimate these model components\footnote{
