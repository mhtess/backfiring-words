...
In the \textbf{baseline} condition, no additional information was provided, and participants were asked: ``Who/what is the person/business related to Person B?'' (e.g., Who is Molly's officemate tutoring?). 
...
In the \textbf{backfiring} condition, Person B utters that the event occurred.
...
As predicted, participants endorsed the likely response \emph{less} in the Backfiring condition (M=-0.09; bootstrapped 95\% CI [-0.15, -0.03]) than the in the Prior condition (M=0.18 [0.14,0.22]), $\beta = -0.24; SE = 0.03; t = -8.3$.
%$\beta = -0.24; SE = 0.03; t(1794) = -8.3; p<0.001$
There was a trend for participants to endorse the likely response \emph{slightly more} in the Observation condition (M=0.24 [0.18, 0.29]) than in the Prior condition ($\beta = 0.05; SE = 0.03; t = 1.73$). 
...
Critically, participants were significantly \emph{less} likely to choose the ``likely''� option in the Backfiring condition than in the Baseline condition ($\beta = -1.42; SE = 0.15; z=-9.0;$).  % p < .001%
Interestingly, however, participants were \emph{no more} likely to choose the ``likely'' option in either the Observation or Instructive conditions than in the Baseline (Figure \ref{fig:expt1-all}; middle panel).
...
Finally, since participants are not given any helpful information in the Baseline condition, we expected the confidence rating data to be lower in the Baseline condition than in the three experimental conditions.
...
This lack of a difference is plausibly due to participants have a strong prior belief that the option that was more likely given the evidence was also more likely \emph{a priori}. 
We observed a baseline probability reliably greater than 50\% for 8 out of the 16 items.
In addition, participants may have believed the observation and instructive vignettes to be suggestions \emph{from the experimenter} that the habit was unlikely, and thus those conditions may have backfired as well (e.g., some participants may have asked themselves ``why is the experimenter going through all this trouble to tell me that the event occurred?'') \mht{cite ransom et al., experimenter effects in reasoning?}.
