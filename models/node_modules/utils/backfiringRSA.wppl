...
var literalBayesian = function(utterance, priorParameters, evidence) {
  Enumerate(function(){

  var habit = flip(priorParameters.habit_rate)
  var habit_strength = sample(priorParameters.habit_prior)

  var cost = priorParameters.cost

  var p = 1-Math.exp(poissonERP.score([habit_strength],0))
  var event_probability = p == 1 ? 0.9999 : p==0 ? 0.0001 : p

  // not habit == delta @ impossible
  var action = habit ? flip(event_probability): flip(0.0001) 

    var m = meaning(utterance, action)
    condition(evidence ?  m : true)

    return Math.log(habit_strength)
  })
}


var listener0 = cache(function(utterance, habit, event_probability) {
  Enumerate(function(){
    var action = habit ? flip(event_probability) : flip(0.0001) 
    var m = meaning(utterance, action)
    condition(m)
...
var speaker1 = cache(function(action, habit, event_probability, cost) {
  Enumerate(function(){
    var utterance = ["It happened.", 
                      "It did not happen.", 
                      "say nothing"][discrete([1,1,cost])]
    var L0 = listener0(utterance, habit, event_probability)
...
var continuous_listener1 = function(utterance, priorParameters, alpha) {
  Enumerate(function(){
  ...
  var S1 = speaker1(action, habit, event_probability, cost)
  factor(alpha*S1.score([],utterance))
