...
var discretize3NormalPrior = function(params, log_scale){
  Enumerate(function(){
     var logRate = flip(params.predictive0) ? 
                flip(params.predictive1) ? 
                priorBins[discrete(gaussianPMF(params.mu_2, params.sigma_2, priorBins))] :
                priorBins[discrete(gaussianPMF(params.mu_3, params.sigma_3, priorBins))] : 
                minBin
    return Math.exp(logRate + log_scale) // returns rate for a given window (e.g. daily rate, monthly rate)
  })
}
